...
def _generate_timepoints(timebase: Decimal, active_channels: int, *args, **kwargs):
    ...
    timepoints = _generate_delayed_timepoints(
        kwargs["previous_timepoints"], delay, parent=kwargs["parent"], time_range=time_range
    )
    return timepoints
...
def _generate_random_noise(timepoints, std_dev, *args, **kwargs):
    rng = np.random.default_rng()
    noise: NDArray = rng.normal(loc=0.0, scale=std_dev, size=timepoints.shape)

    return noise.astype(_dtype)
...
def get_waveform(*args, **kwargs):
    ...
    # if connector is unplugged return noise
    if "connector_state" not in kwargs or not kwargs["connector_state"]:
        t = _generate_timepoints(*args, **kwargs)
        if "noise_std_dev" in kwargs:
            std_dev = kwargs["noise_std_dev"]
            noise = _generate_random_noise(t, std_dev, *args, **kwargs)
            return t, noise, noise
        else:
            return t, np.zeros_like(t), np.zeros_like(t)

    if kwargs["waveform"] == "sine":
        return _generate_sine(*args, **kwargs)
    elif kwargs["waveform"] == "square":
        return _generate_square(*args, **kwargs)
    elif kwargs["waveform"] == "triangle":
        return _generate_triangle(*args, **kwargs)
    elif kwargs["waveform"] == "sawtooth":
        return _generate_sawtooth(*args, **kwargs)
    elif kwargs["waveform"] == "pulse_train":
        return _generate_pulse_train(*args, **kwargs)
    elif kwargs["waveform"] == "pulse_train_conv":
        return _generate_pulse_train_convolution(*args, **kwargs)
    else:
...
class SignalGenerator(QObject):
    ...
    def initialize_waveform(self, *args, **kwargs):
        result = get_waveform(*args, **kwargs)
        if not result:
            print("result is None")
            return
        self.t, self.wfm, self.noise = result
...
if __name__ == "__main__":
    # test
    # f = 50E6
    # tb = 20E-9  # at 1E-2 1ns pulses at 88MHz repetition rate are gone! Artifacts show up to the rightmost peak
